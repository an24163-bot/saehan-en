...
(function(){
  ...
  var html = css +
    '<div id="snt-fab" class="snt-fab" role="region" aria-label="Quick contact">' +
      ...
      '</button>' +
    '</div>' +
    '<div id="snt-wechat-modal" class="snt-wcm" role="dialog" aria-label="Connect on WeChat" onclick="if(event.target===this)this.classList.remove(\'is-open\');">' +
      '<div class="snt-wcm__box">' +
        '<button class="snt-wcm__close" onclick="document.getElementById(\'snt-wechat-modal\').classList.remove(\'is-open\');" aria-label="Close">×</button>' +
        '<h3 class="snt-wcm__title">Connect on WeChat</h3>' +
        '<p class="snt-wcm__desc">Scan QR code or search our WeChat ID:</p>' +
